  ...
  try {
    ...
    if (post) {
      ...
      return res
        ...
        .json({ msg: "Post Sucessfully uploaded" });
    }
...
const getAllPosts = async (req, res) => {
  try {
    ...
    if (userPost)
      return res.status(STATUSCODE.success).json({ userPost: userPost });
...
const deletePost = async (req, res) => {
  try {
    ...
    if (isDelete)
      return res.status(STATUSCODE.success).json({ msg: "Post Deleted" });
    return res
      .status(STATUSCODE.success)
      .json({ msg: "Something Wrong ! This Post Not found" });
  } catch (e) {
    console.log(e);
    return res.status(STATUSCODE.internal).json({
      msg: "something wrong",
